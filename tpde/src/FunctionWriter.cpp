...
void FunctionWriterBase::begin_module(Assembler &assembler) {
...
void FunctionWriterBase::end_module() {
...
FunctionWriterBase::JumpTable &
    FunctionWriterBase::alloc_jump_table(u32 size, Reg idx, Reg tmp) {
...
void FunctionWriterBase::eh_align_frame() {
...
void FunctionWriterBase::eh_write_inst(const u8 opcode, const u64 arg) {
...
void FunctionWriterBase::eh_write_inst(const u8 opcode,
                                       const u64 first_arg,
                                       const u64 second_arg) {
  eh_write_inst(opcode, first_arg);
  eh_writer.write_uleb(second_arg);
}

void FunctionWriterBase::eh_advance_raw(u64 size_units) {
  if (size_units < 0x40) {
...
void FunctionWriterBase::eh_init_cie(SymRef personality_func_addr) {
...
void FunctionWriterBase::eh_begin_fde(SymRef personality_func_addr) {
...
void FunctionWriterBase::eh_end_fde() {
  eh_align_frame();
...
void FunctionWriterBase::except_encode_func() {
  if (!cur_personality_func_addr.valid()) {
    return;
  }
...
void FunctionWriterBase::except_add_cleanup_action() {
...
void FunctionWriterBase::except_add_empty_spec_action(const bool first_action) {
  if (!first_action) {
    except_action_table.back() = 1;
  }
...
u32 FunctionWriterBase::except_type_idx_for_sym(const SymRef sym) {
